...
public class EnderChestManager extends Manager<VariableEnderChests> implements Listener {
        ...
        plugin.getServer().getGlobalRegionScheduler().runAtFixedRate(
            ...
            for (UUID uuid : toRemove) {
                ...
                this.getPlugin().getLogger().info("Saving data for " + enderChest.getName());
                    ...
                    this.getPlugin().getServer().getAsyncScheduler().runNow(
                            this.getPlugin(),
                            t -> this.dataManager.saveEnderChestMultiple(toSave)
                    );
            ...
            if (enderChest == null) {
                ...
                this.getPlugin().getLogger().info("Loaded enderchest for " + name);
                    ...
                    event.getPlayer().getScheduler().run(
                            getPlugin(),
                            t -> uuidEnderChestMap.put(
                                    uuid,
                                    enderChest == null ? createNew(event.getPlayer()) : enderChest
                            ),
                            null
                    );
        });
    ...
    public Callback<EnderChest> getEnderChest(String name) {
        ...
        getPlugin().getServer().getAsyncScheduler().runNow(
                getPlugin(),
                task -> {
            ...
            EnderChest enderChest = snapshot == null ? null : new EnderChest(snapshot.getUuid(), snapshot.getName(), snapshot.getContents(), snapshot.getRows());
                    this.getPlugin().getServer().getGlobalRegionScheduler().run(
                            this.getPlugin(),
                            t -> {
    ...
    public Callback<EnderChest> getEnderChest(UUID uuid) {
        ...
        getPlugin().getServer().getAsyncScheduler().runNow(
            ...
            EnderChest enderChest = snapshot == null ? null : new EnderChest(snapshot.getUuid(), snapshot.getName(), snapshot.getContents(), snapshot.getRows());
                    getPlugin().getServer().getGlobalRegionScheduler().run(
                            getPlugin(),
                            t -> {
